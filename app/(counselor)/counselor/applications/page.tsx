...
export default function CounselorApplicationsPage() {
  ...
  // Get counselor id once on mount
  useEffect(() => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const result = await applicationRepository.searchCounselorApplications({
        counselorId,
  ...
  }, [counselorId, debouncedSearch, statusFilter, page]);
      ...
      <div className="w-full max-w-6xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-primary-500 mt-0.5 text-sm">
            {loading
              ? "Loading…"
              : `${totalCount} application${totalCount !== 1 ? "s" : ""} assigned to you`}
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:items-center">
          <div className="relative flex-1 min-w-0">
            ...
            <input
              type="text"
              placeholder="Search by student, program, university..."
              value={search}
            ...
            <div className="bg-white w-full rounded-2xl p-10 text-center border border-primary-100 shadow-sm">
              ...
              <p className="text-primary-500 text-sm max-w-xs mx-auto">
                {search
                  ...
                  : "You have no applications assigned yet."}
              </p>
            </div>
          ) : (
            <div className="flex flex-col gap-3 w-full">
              <AnimatePresence mode="popLayout">
                {data.map((app) => (
                  ...
                  >
                    <Link
                      href={`/counselor/applications/${app.id}`}
