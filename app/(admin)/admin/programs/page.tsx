...
export default function AdminProgramsPage() {
  ...
  const columns: Column<Program>[] = [
    ...
    {
      header: "Tuition",
      accessor: "tuition_after_scholarship",
      render: (row) => (
        <span className="text-sm text-primary-600">
          {row.tuition_after_scholarship
            ? `¥${row.tuition_after_scholarship.toLocaleString()}`
            : "—"}
        </span>
      ),
    },
...
actions={(row) => (
          <>
            <Link
              href={`/universities/${(row as any).university?.slug}/${row.slug}`}
              target="_blank"
            ...
            <Link
              href={`/admin/programs/${row.id}/edit`}
              className="p-2 text-primary-400 hover:text-brand-600 transition-colors"
              title="Edit"
            >
