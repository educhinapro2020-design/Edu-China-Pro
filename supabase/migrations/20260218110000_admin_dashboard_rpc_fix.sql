...
** Migration: 20260218110000_admin_dashboard_rpc_fix
** Description: Fix application & signup trend to group by day (not week)
**              with zero-filled gaps using generate_series.
**              Fix apps_by_degree to always return all 4 degree levels
**              even when there are zero applications for that degree.
*/
...
BEGIN
  ...
  -- ── Signup Trend (daily, zero-filled) ───────────────────────
  ...
  FROM (
    ...
    FROM generate_series(v_start::DATE, NOW()::DATE, '1 day'::INTERVAL) AS gs(day)
    LEFT JOIN (
      SELECT
  ...
  -- ── Application Trend (daily, zero-filled) ──────────────────
  SELECT COALESCE(jsonb_agg(row_to_json(t)::JSONB ORDER BY t.day), '[]'::JSONB)
  ...
  -- ── Applications by Degree (all 4 levels, zero-filled) ──────
  -- Uses a static VALUES list so degrees with 0 applications still appear
