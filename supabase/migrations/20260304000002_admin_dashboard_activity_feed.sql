/*
** Date: 2026-03-04
** Migration: 20260304000002_admin_dashboard_activity_feed
** Description: Add recent_activity feed to get_admin_dashboard RPC.
**              Replaces status_movements with last 10 status changes
**              including student name and clickable application_id.
*/
...
BEGIN
  ...
  SELECT count(*) INTO v_needs_action FROM applications WHERE status = 'action_required';
  SELECT count(*) INTO v_pending_review FROM applications WHERE status IN ('submitted', 'reviewing');
  SELECT count(*) INTO v_visa_granted FROM applications WHERE status = 'visa_granted';
  SELECT count(*) INTO v_unassigned FROM applications
    WHERE counselor_id IS NULL
      AND status NOT IN ('draft', 'withdrawn', 'closed', 'dropped_off');
  SELECT count(*) INTO v_students_this_month FROM profiles
    WHERE role = 'student' AND created_at >= v_month_start;
  SELECT count(*) INTO v_apps_this_month FROM applications
    WHERE created_at >= v_month_start;

  SELECT COALESCE(jsonb_agg(row_to_json(t)::JSONB ORDER BY t.sort_order), '[]'::JSONB)
  INTO v_apps_by_status
  FROM (
    SELECT s.status AS label, s.sort_order, s.stage, COALESCE(a.value, 0) AS value
    FROM (
      VALUES
        ('draft',                    1,  'early'),
        ('submitted',                2,  'early'),
  ...
  FROM (
    SELECT d.label, COALESCE(a.value, 0) AS value
    ...
    LEFT JOIN (
      ...
      FROM applications a JOIN programs p ON p.id = a.program_id GROUP BY p.degree_level
  ...
  FROM (
    SELECT pr.full_name AS counselor_name, count(*)::INT AS active_count
    FROM applications a
  ...
  SELECT COALESCE(jsonb_agg(row_to_json(t)::JSONB ORDER BY t.changed_at DESC), '[]'::JSONB)
